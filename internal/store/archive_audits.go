...
func (s *FileStore) ArchiveVerifications(caseID string) ([]archive.VerificationRecord, error) {
	...
	lock.Lock()
	defer lock.Unlock()
	reader, err := s.archiveAuditReader(caseID)
...
func (s *FileStore) archiveAuditReader(caseID string) (*os.File, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	if reader := s.auditReaders[caseID]; reader != nil {
		return reader, nil
	}
	reader, err := os.Open(s.archiveAuditPath(caseID))
	if err != nil {
		return nil, err
	}
	s.auditReaders[caseID] = reader
	return reader, nil
}
